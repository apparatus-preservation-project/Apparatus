...
public class SoundManager {
	...
	public static void tick() {
		if (init_sound && Game.enable_sound) {
			if (!RocketEngine.lights.isEmpty()) {
				snd_rocket_thrust.setVolume(0.2f + ((((float) Math.min(RocketEngine.lights.size(), 4)) / 4.0f) * 0.8f));
			}
			...
			if (num_rolling > 0) {
				...
				for (int x = 0; x < num_rolling; x++) {
					volume += Math.min(rolling[x].body.getLinearVelocity().len2() / 100.0f, 1.0f);
				}
				snd_marble_roll.setVolume((volume / ((float) num_rolling)) * 0.8f);
			}
		}
	}
	...
	public static void handle_gift_hit(ChristmasGift m, GrabableObject b) {
		if (init_sound && Game.enable_sound) {
			...
			if (now - last_gift_hit >= 80) {
				...
				snd_gift_hit[x].play(0.8f * Math.min((tmp.len2() * 2.0f) / 70.0f, 0.7f));
				last_gift_hit = now;
			}
		}
	}
	...
	public static void play_wood_hit(float vol) {
		if (init_sound && Game.enable_sound) {
			snd_wood_hit[_r.nextInt(snd_wood_hit.length)].play(0.7f * vol);
		}
	}
