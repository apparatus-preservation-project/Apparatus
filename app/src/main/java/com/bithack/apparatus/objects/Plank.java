...
public class Plank extends Bar implements FreeObject {
	...
	public static void init_materials(boolean shadowed) {
		...
		G.gl.glMaterialfv(1032, 4609, _material, 0);
		G.gl.glMaterialfv(1032, 4610, _material, 0);
	...
	public static void init_dark_material() {
		G.gl.glMaterialfv(1032, 4609, _material, 13);
		G.gl.glMaterialfv(1032, 4610, _material, 17);
		G.gl.glMaterialfv(1032, 5633, _material, 21);
	}
	...
	public Vector2 check_sensor_intersection(Fixture a, Plank gb) {
		...
		float f2 = gb.size.x / 2.0f;
		float angle = get_angle();
		_dir.set((float) Math.cos((double) angle), (float) Math.sin((double) angle));
