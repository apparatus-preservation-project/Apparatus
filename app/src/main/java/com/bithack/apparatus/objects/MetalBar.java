...
public class MetalBar extends Bar {
	...
	public MetalBar(World world2) {
		...
		this.body = world2.createBody(_bd);
	...
	public void render() {
		...
		float angle = s.angle;
		G.gl.glPushMatrix();
		G.gl.glTranslatef(pos.x, pos.y, (((float) this.layer) * 1.5f) + 0.5f);
		G.gl.glRotatef((float) Math.toDegrees((double) angle), 0.0f, 0.0f, 1.0f);
		if (this.layer == 1) {
	...
	public void tick() {
		if (this.pending_corner != null) {
			basepos1.add(dist);
			double angle = ((double) this.pending_corner.body.getAngle()) % 6.283185307179586d;
			double angle2 = ((double) this.body.getAngle()) % 6.283185307179586d;
			double adist = Math.abs((((double) this.pending_corner.body.getAngle()) % 6.283185307179586d) - ((((double) this.body.getAngle()) % 3.141592653589793d) * 2.0d));
			double pdist = (double) basepos1.dst(basepos2);
	...
	public void get_sensor_dist(Vector2 a, Vector2 b, MetalBar o, Vector2 out) {
		out.set(b);
		out.x *= o.size.x / 2.0f;
		out.y *= o.size.y / 2.0f;
	...
	/* access modifiers changed from: protected */
	public int get_adj_sensor_world_pos(Vector2 p, Vector2 out) {
		Vector2 tmp = null;
		int side = -1;
