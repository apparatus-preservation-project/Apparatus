...
public class Weight extends GrabableObject {
	...
	public final void draw_shadow_volume(Vector3 lightdir) {
		...
		float z = 0.125f + ((float) this.layer);
		if (((double) Math.abs(angle - this.last_angle)) > 0.01d) {
			this.silhouette_mesh = _silhouette.transform_mark_backfaces(tmp2.x, tmp2.y, z, 2.0f, 1.0f, 1.0f, 57.29578f * get_angle(), lightdir).generate_mesh(this.silhouette_mesh, lightdir, this.layer);
