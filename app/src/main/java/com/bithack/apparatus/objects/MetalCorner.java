...
public class MetalCorner extends GrabableObject {
	...
	MetalBar m_a;
	MetalBar m_b;
	private boolean shadow_created = false;
	...
	public MetalCorner(World world, MetalBar a, MetalBar b, Vector2 cornertop, Vector2 basepos1, Vector2 basepos2, int sidea, int sideb) {
		...
		this.height = (float) Math.sqrt((double) (1.0f - ((this.baselen / 2.0f) * (this.baselen / 2.0f))));
		this.angle = (float) Math.atan2((double) basepos2.y, (double) basepos2.x);
		if (cornertop.x == cornertop.y) {
			this.angle = (float) (((double) this.angle) - 3.141592653589793d);
		}
		this.b1_id = a.__unique_id;
		this.b2_id = b.__unique_id;
		init_properties();
	...
	public static void _init() {
		if (!_initialized) {
			...
			_fd = new FixtureDef();
			_fd.shape = _shape;
			_mesh = new Mesh(true, 9, 0, new VertexAttribute(1, 3, "a_position"), new VertexAttribute(8, 3, "a_normal"), new VertexAttribute(16, 2, "a_texcoords"));
	...
	public void render() {
		G.gl.glPushMatrix();
		G.gl.glTranslatef(this.center.x, this.center.y, (float) this.layer);
		G.gl.glRotatef((float) (((double) this.angle) * 57.29577951308232d), 0.0f, 0.0f, 1.0f);
	...
	@Override // com.bithack.apparatus.objects.GrabableObject, com.bithack.apparatus.objects.BaseObject
	public void set_angle(float angle2) {
	}
