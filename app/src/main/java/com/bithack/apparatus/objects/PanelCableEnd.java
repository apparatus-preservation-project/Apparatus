...
public class PanelCableEnd extends GrabableObject implements BaseCableEnd {
	...
	public PanelCableEnd(World world2, PanelCable cable2) {
		if (!_initialized) {
			_init();
		}
		this.world = world2;
		this.cable = cable2;
		this.body = world2.createBody(Rope._bd);
		_fd.isSensor = true;
	...
	private void fix_anchor_pos() {
		if (this.attached_type == 1) {
			if (this.attached_battery.size == 1) {
				this.anchor.set(this.attached_battery.body.getWorldPoint(tmp.set(-0.0f + (((float) this.socket) * 0.3f), -0.3f)));
			} else {
				this.anchor.set(this.attached_battery.body.getWorldPoint(tmp.set(-0.2f + (((float) this.socket) * 0.4f), -0.6f)));
			}
	...
	public void translate(float x, float y) {
		if (!this.attached) {
			super.translate(x, y);
		} else if (this.anchor.dst(x, y) > 3.5f) {
			detach();
			super.translate(x, y);
		} else if (this.attached_rengine == null && this.anchor.dst(x, y) > 2.5f) {
			detach();
			super.translate(x, y);
		} else if (this.attached_type == 2) {
