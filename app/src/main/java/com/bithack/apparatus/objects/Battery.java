...
public class Battery extends GrabableObject {
	...
	public final void render() {
		if (!this.culled) {
			...
			G.gl.glPushMatrix();
			G.gl.glTranslatef(pos.x, pos.y, 0.5f + ((float) this.layer));
	...
	public void render_btn() {
		if (!this.culled && this.size == 2) {
			...
			G.gl.glRotatef((float) (this.on ? 15 : -15), 0.0f, 0.0f, 1.0f);
	...
	public void render_deco(Camera cam) {
		if (!this.culled) {
			...
			tmpmat.setToRotation(G.vec_rot, (float) (((double) angle) * 57.29577951308232d));
