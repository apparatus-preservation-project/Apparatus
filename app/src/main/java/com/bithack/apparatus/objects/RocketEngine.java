...
public class RocketEngine extends GrabableObject {
	...
	public void step(float deltatime) {
		if (this.active) {
			this.counter++;
			if (this.output > 0.0f) {
				float angle = (float) (((double) get_state().angle) + 1.5707963267948966d);
				tmp.set((float) (Math.cos((double) angle) * 500.0d * ((double) this.thrust) * ((double) this.output)), (float) (Math.sin((double) angle) * 500.0d * ((double) this.thrust) * ((double) this.output)));
	...
	public static class Fire {
		...
		public Fire(int p, Vector2 origo, float angle) {
			this.pos.set(origo);
			this.pos.x += (RocketEngine._r.nextFloat() - 16.0f) * 0.5f;
			this.pos.y += (RocketEngine._r.nextFloat() - 16.0f) * 0.5f;
			this.time = 0.0f;
