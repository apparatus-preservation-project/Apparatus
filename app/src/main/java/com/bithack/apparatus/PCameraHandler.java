...
public class PCameraHandler {
	...
	public PCameraHandler(PerspectiveCamera camera2) {
		this.camera = camera2;
		this.camera_pos.set(camera2.position);
		this.camera_direction.set(camera2.direction);
	}

	public void set_target(BaseObject target2, Vector3 offs) {
		this.target = target2;
		this.target_offs.set(offs.x, offs.y, 0.0f);
	}

	public void set_target(BaseObject target2, float offs_x, float offs_y) {
		this.target = target2;
	...
	public void tick(float delta, boolean update) {
		if (this.target != null) {
			...
			this.velocity.mul(Math.max(1.0f - (((G.delta / 4.0f) * 18.0f) * ((100.0f - ((float) 10)) / 100.0f)), 0.0f));
			...
			Intersector.intersectRayPlane(G.p_cam.getPickRay((float) (G.realwidth / 2), (float) (G.realheight / 2)), Game.yaxis, this.tmp2);
			...
			this.velocity.mul(Math.max(1.0f - (((G.delta / 4.0f) * 18.0f) * ((100.0f - ((float) Math.min(80, Game.camera_smoothness))) / 100.0f)), 0.0f));
			if (this.velocity.len() < 0.1f) {
				this.velocity.set(0.0f, 0.0f, 0.0f);
			}
