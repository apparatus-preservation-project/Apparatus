...
public class G {
	...
	public static void reset_cam() {
		cam.viewportHeight = ((float) height) / 16.0f;
		cam.viewportWidth = ((float) width) / 16.0f;
	...
	public static void init(int width2, int height2) {
		...
		cam = new OrthographicCamera(((float) width) / 16.0f, ((float) height) / 16.0f);
		cam.position.set(0.0f, 0.0f, 0.0f);
		cam.update();
		cam_p = new OrthographicCamera((float) width, (float) height);
		cam_p.position.set((float) (width / 2), (float) (height / 2), 0.0f);
		cam_p.far = 500.0f;
		cam_p.update();
		p_cam = new PerspectiveCamera(45.0f, ((float) width) / 16.0f, ((float) height) / 16.0f);
